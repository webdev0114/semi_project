...
<head>
  ...
  <link rel="stylesheet" href="<%=request.getContextPath()%>/food/review.css">
	...
	<div class="container">
			...
			<div class="col-12"><br>
				...
				<c:if test="${!empty sessionScope.id && sessionScope.seller eq 'seller' }"> 
						<div class="text-right">
							...
							onclick="location.href='<%=request.getContextPath()%>/F_BoardForm.f' ">
		...
		<c:forEach var="food" items="${foodlist }">	
			<li class="review-item">
				<div class="preview">
					<img title="맛집 이미지" alt="맛집 이미지" src="./p_file/${food.f_file}"
				...
				<div class="review-title">
					<a href="./F_DetailAction.f?f_num=${food.f_num}&page=${page}">
					${num}. ${food.f_name }
					</a>	
				</div>
				<c:set var="num" value="${num+1}"/>
					<div class="review-content">&emsp;
						<span class="food"><textarea class="textbar" cols="100" rows="5" readonly>${food.f_content}</textarea></span>
					</div>		
				</div>
			</li>
			<hr size="80px">
...
<!-- 1페이지로 이동 -->
<li class="page-item">
	       <a class="page-link" href="./F_ListAction.f?page=1" style="text-decoration:none" tabindex="-1" aria-disabled="true"><<</a>
		</li>
<!-- 이전 블럭으로 이동 -->
<c:if test="${startPage > 10 }">
	<li class="page-link">
			<a href="./F_ListAction.f?page=${startPage-10 }">[이전]</a>
			</li>
</c:if>
...
<c:forEach var="i" begin="${startPage }" end="${endPage }">
	...
	<c:if test="${i != page }">
		<li class="page-item">
			<a class="page-link" href="./F_ListAction.f?page=${i}">${i}</a>
				</li>
	</c:if>
</c:forEach>
...
<c:if test="${endPage < pageCount }">
	<a href="./F_ListAction.f?page=${startPage+10 }">[다음]</a>
</c:if>

<!-- 마지막 페이지로 이동 -->
		<li class="page-item"> 
			<a class="page-link" href="./F_ListAction.f?page=${pageCount}" style="text-decoration:none">>></a>
			</li>
		</ul>
	</nav>
</c:if>
