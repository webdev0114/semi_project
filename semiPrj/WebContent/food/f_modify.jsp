		...
		<br><br><br>
			...
			<div class="row gutter-v1 align-items-stretch mb-5">
 			<div class="col-12">
				...
				<form class="form-signin" action="<%=request.getContextPath()%>/F_Modify.f" method="post"
					enctype="multipart/form-data">
				<div class="text-left">
			          	<h2 class="section-title">맛집 글수정</h2>
				</div>
				...
				<div class="mb-3">
					<input type="id" class="form-control" name="id" id="id" placeholder="${id}" readonly>
				</div>
				
				<div class="mb-3">
					<input type="text" class="form-control" name="f_name" id="f_name" value="${food.f_name }" placeholder="음식점 이름"/>
				</div>
			
      			<div class="mb-3">
					<input type="password" class="form-control" name="f_pw" id="f_pw" placeholder="비밀번호">
				</div>
      			
				<div class="mb-3">
					<textarea class="form-control" name="f_content" id="f_content"
						cols="67" rows="15">${food.f_content}</textarea>
				</div>
				
				<div class="mb-3">
					<input type="text" class="form-control" name="f_addr" id="f_addr"
						value="${food.f_addr}" size="50">
				</div>

				<div class="mb-3">
					<input class="form-control" type="file" name="f_file" id="f_file">
				</div>
				
				<hr class="mb-4">
				<div class="text-left">
					<button class="btn btn-light btn-sm" type="submit">수정</button>
					<button type=button class="btn btn-light btn-sm"
			       		onClick="location.href='./F_ListAction.f'">목록</button>
				</div>
				</form>
				</div>
			</div>
	...
	<script>
		$(document).ready(function() {
			$("form").submit(function() {
				...
				if ($("#f_content").val() == "") {
					alert("음식점 소개를 입력해 주세요.");
					$("#f_content").focus();
					return false;
				}
				if ($("#f_addr").val() == "") {
					alert("음식점 주소를 입력해 주세요.");
					$("#f_addr").focus();
					return false;
				}
			});
