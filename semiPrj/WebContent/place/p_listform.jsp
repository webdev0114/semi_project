...
<div class="place-wrapper">
	<div class="container">
		<div class="row gutter-v1 align-items-stretch mb-5">
			<div class="col-12"><br>
				<h2 class="section-title">여행</h2>
			
			<c:if test="${!empty sessionScope.id && sessionScope.seller eq 'seller' }"> 
						<div class="text-right">
							<button class="btn btn-default btn-sm" type=submit
							onclick="location.href='<%=request.getContextPath()%>/P_WriteForm.p' ">
		...
		<c:forEach var="place" items="${boardlist }">	
		
			<li class="review-item">
				<div class="preview">
					<img title="여행 이미지" alt="여행 이미지" src="./p_file/${place.p_file}"
				     	width="100" height="100">
				</div>
				
				&emsp;<div class="review-title contents">
					<div>작성자 : ${place.id }</div>
					
				<div class="review-title">
					<a href="./P_DetailAction.p?p_num=${place.p_num}&page=${page}">
							${num}. ${place.p_name }
					</a>
				</div>
				<c:set var="num" value="${num+1}"/>
					<div class="review-content">&emsp;	
						<span class="reivew"><textarea class="textbar" cols="100" rows="5" readonly>${place.p_content}</textarea></span>			
					</div>
					</div>
				</li>
			<hr size="80px"><br>	
...
<c:if test="${listcount>0 }">
	<nav aria-label="Page navigation example">
		...
		<!-- 1페이지로 이동 -->
		<li class="page-item">
			 <a class="page-link" href="./P_ListAction.p?page=1" style="text-decoration:none" tabindex="-1" aria-disabled="true"><<</a>
		</li>

		<!-- 이전 블럭으로 이동 -->
		<c:if test="${startPage > 10 }">
			<li class="page-link">
			<a href="./P_ListAction.p?page=${startPage-10}">[이전]</a>
			</li>
		</c:if>
		...
		<c:forEach var="i" begin="${startPage}" end="${endPage}">
			...
			<c:if test="${i != page}">	<!-- 현재 페이지가 아닌 경우 -->
				<li class="page-item">
					<a class="page-link" href="./P_ListAction.p?page=${i}">${i}</a>
				</li>
			</c:if>
		</c:forEach>
...
<!-- 다음 블럭으로 이동  -->
	<c:if test="${endPage < pageCount}">
		<a href="./P_ListAction.p?page=${startPage+10}">[다음]</a>
	</c:if>
	
<!-- 마지막 페이지로 이동  -->

			<li class="page-item"> 
				<a class="page-link" href="./P_ListAction.p?page=${pageCount}" style="text-decoration:none">>></a>
			</li>
		</ul>
	</nav>
</c:if>
