  ...
  <body>
	...
	<div class="container">
		<div class="row gutter-v1 align-items-stretch mb-5">
			<div class="col-12">
					<form class="form-signin" action="<%=request.getContextPath() %>/P_Modify.p" method="post"
						enctype="multipart/form-data">
			<input type="hidden" name="p_num" value="${p.p_num }">
			<input type="hidden" name="page" value="${page }">			
			<div class="text-left">
          		<h2 class="section-title">여행 게시판 수정</h2>
          	</div>
		
		<div class="mb-3">
			<input type="text" class="form-control" name="p_name" id="p_name" value="${p.p_name}" placeholder="상품이름">
 		</div>
	
  		<div class="mb-3">
			<input type="text" class="form-control" name="id" id="id" placeholder="${p.id}" readonly>
		</div>
      
       	<div class="mb-3">
			<input type="password" class="form-control" name="p_pw" id="p_pw" placeholder="비밀번호">
		</div>
      
      	<div class="mb-3">
			<textarea class="form-control" name="p_content" id="p_content"  cols="67" rows="15">${p.p_content}</textarea>
		</div>
      
      <div class="mb-3">
        <input type="text" class="form-control" name="p_addr" id="p_addr" value="${p.p_addr}" size="50" placeholder="여행지 주소">
      </div>
